...
typedef struct {
	...
	void		*atomizer;	/* a token representing the atomizer */
} atomizer_find_or_make_atomizer;

typedef struct {
	uint32		magic;	/* magic number to make sure the caller groks us */
	void		*atomizer;	/* a token representing the atomizer to delete */
...
typedef struct {
	...
	void		*atom;		/* token representing atomized string */
} atomizer_atomize;
